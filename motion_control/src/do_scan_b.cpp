...
void do_scan ( float rotation_deg, float x_s, float y_s, float z_s, float x_e, float y_e, float z_e, float x_final, float y_final, float z_final  )
{
  ...
  if ( success )
  {
    ...
    target_pose2.position.x = target_pose2.position.x + 0.045;
    waypoints.push_back ( target_pose2 );

    target_pose1.position.x = target_pose1.position.x + 0.045;
    waypoints.push_back ( target_pose1 );

    std_srvs::Empty msg;
    double fraction = get_trajectory ( waypoints, move_group, my_plan, 0.035 );
    if ( fraction > 0.98 )
    {
